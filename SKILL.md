...
- **第一层 核心流程（每轮必跑）**：触发守卫 → 攻击循环（暴露靶子于分析前；强制攻击/修订痕迹/承认局限于交付前）→ 五阶段（分解/搜索/透镜选择纪律/工具应用/合成）→ 推演四查 → 元认知三项（敌对测试/数据依赖审计/盲点验证）→ 交付前重读
...
对抗单一叙事先天倾向——先验在分析开始前即已形成，且任何自我检验机制（先验声明/证伪条件/信源自查）均不可解：检验者与被检验者是同一系统（内省不可靠、证伪可被叙事吸收、信源可被环境过滤）。唯一被实证起效的对抗是"来自不同立场的攻击"（对话中的外部质疑能真实改变先验）。本协议不宣称消除先验（不可解），只强制"先验暴露并被攻击"：

1. **暴露靶子**（触发守卫后执行）：简述本分析的默认立场 + 关键结论清单——目的不是自证中立（不可能），是给攻击提供靶子。第一反应必须具体（如"默认'仅后训练'叙事为真"），禁止"保持开放"式敷衍。
2. **强制攻击**（交付前执行）：执行一轮对立立场攻击——至少一个真实或模拟的相反立场，逐条攻击关键结论。攻击质量由"是否触及默认假设"衡量，而非攻击数量。攻击来源：用户质疑、模拟对立立场、历史反例。
3. **修订痕迹**（产物出口 = 位置性标注）：交付中必须可见"哪些结论因攻击被修改、哪些被拒绝及拒绝理由"——这是唯一外部可验证的产物（攻击痕迹可被检验；先验/证伪不可被检验）。
4. **承认局限**（交付前）：声明"本分析接受了何种攻击、何种攻击无法在本系统内完成（如无真实外部先验）"——把自我指涉的局限显式化，而非假装解决。

**多元是搜索空间，不是呈现格式**——竞争假说按证据权重排序，禁止对称并列（与虚假平衡禁止一致）。与共存检查（补充①）对象不同：共存检查怀疑"变化叙事"，本协议怀疑"立场叙事"——不冲突。未执行暴露靶子或强制攻击的分析不得进入交付。
