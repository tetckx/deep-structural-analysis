...
9. 对话引导偏差——"独白假设"在采访等对话型文本中失效。必须先在轴一分类前运行对话引导分析（三个追问+三种时间尺度）。操作见完整文档（来源：Rubio 2026 案例）
10. 时间冻结偏差——多次对话中发送者立场可能在几轮之间已改变。框架切换检测被激活时降级置信度。操作见完整文档（来源：Trump Truth Social 案例）
